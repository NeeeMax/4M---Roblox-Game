...
| `AutomationCatch` | RemoteEvent | S→C | `source: AutomationSource ("Basket" \| "Intern"), projectileId: number, reward: number` — the basket or the intern caught one of the player's sticks (already paid by the server, `AutomationService`); drives the catch visuals and coin popup | — (server → client) | Marco |
